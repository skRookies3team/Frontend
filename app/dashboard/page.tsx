...
import { DiaryCarousel3D } from "@/components/diary-carousel-3d"
...
const initialTodos: TodoItem[] = [
  { id: "1", text: "아침 산책하기", completed: false },
...
export default function DashboardPage() {
  ...
  const mainPet = user.pets?.[0]
  const hasPets = user.pets && user.pets.length > 0

  const activityPercentage = (healthData.activity.current / healthData.activity.goal) * 100
              ...
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Link href="/missing-pet/register">
                      <Button
                        size="sm"
                    ...
                    >
                      <ChevronRight
                        className={`h-4 w-4 transition-transform duration-200 ${
                          isMissingPetsExpanded ? "rotate-90" : ""
                        }`}
                      />
                    </Button>
                  </div>
