...
export default function ChatbotPage() {
  ...
  const pets = user?.pets || [
    { id: 1, name: "초코", breed: "골든 리트리버", avatar: "/golden-retriever.png", age: "3살", personality: "활발하고 사교적", emoji: "🐕" },
    { id: 2, name: "모카", breed: "푸들", avatar: "/fluffy-white-poodle.png", age: "2살", personality: "영리하고 장난기 많음", emoji: "🐩" },
  ]
  
  const initialPet = petIdFromUrl 
    ? pets.find(p => p.id === Number(petIdFromUrl)) || pets[0]
    : pets[0]
  ...
  useEffect(() => {
    if (petIdFromUrl) {
      const pet = pets.find(p => p.id === Number(petIdFromUrl))
      if (pet) {
        setSelectedPet(pet)
      }
    }
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <Avatar className={`h-14 w-14 border-2 ${
                    selectedPet.id === pet.id ? 'border-white' : 'border-pink-200'
                  }`}>
