  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white pb-20 md:pb-0">
      ...
      <div className="container mx-auto max-w-6xl px-4 -mt-8">
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-4 md:p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/shop/cart" className="relative">
              ...
              {cartCount > 0 && (
                <Badge className="absolute -right-1 -top-1 h-5 w-5 rounded-full bg-gradient-to-r from-pink-500 to-rose-500 p-0 text-xs">
                  {cartCount}
                </Badge>
              )}
            </Link>
          ...
          <div className="relative max-w-2xl mb-4">
            ...
            <Input
              ...
              className="rounded-full border-pink-200 pl-10 focus-visible:ring-pink-500 bg-white"
            />
          </div>
        ...
        <div className="mb-6 rounded-2xl bg-gradient-to-r from-amber-100 via-yellow-100 to-orange-100 p-6 shadow-lg border border-amber-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-pink-600 md:text-base">내 마일리지</p>
              <p className="text-2xl font-bold text-pink-600 md:text-3xl">1,250 포인트</p>
            </div>
            <Link href="/profile/mileage">
              <Button
                variant="outline"
                size="sm"
                className="rounded-full border-pink-500 bg-transparent text-pink-600 hover:bg-pink-500 hover:text-white"
              >
                내역 보기
              </Button>
            </Link>
        ...
        <main className="pb-6">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 md:gap-6">
            {filteredProducts.map((product) => (
              <ProductCard key={product.id} product={product} onAddToCart={() => setCartCount(cartCount + 1)} />
            ))}
          </div>

          {filteredProducts.length === 0 && (
            <div className="py-12 text-center">
              <p className="text-muted-foreground md:text-lg">상품을 찾을 수 없어요</p>
            </div>
          )}
        </main>
      </div>
